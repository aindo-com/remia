...
def main():
    ...
    # specify a generator
    if NAME_GENERATOR.startswith("leak"):
        from src.generators import leak

        leak_frac = float(NAME_GENERATOR.split("_")[-1])

        # get the remaining data that is not in aux or test to be used as additional data for the leaky generator
        df_add = df[len(df_aux) + len(df_test) :]

        if len(df_add) < N_SYNTHETIC:
            raise ValueError(
                f"Not enough additional data for the leaky generator. Required: {N_SYNTHETIC}, available: {len(df_add)}."
            )

        # remove the target record from the additional data if it is in there by checking the values
        cols_equal_to_target = (
            df_add[df_add.columns].values == target_record[df_add.columns].values
        ).sum(axis=1)
        df_add = df_add[cols_equal_to_target != len(df_add.columns)]

        generator = leak(additional_data=df_add, leak_frac=leak_frac)
    else:
    ...
    datasets_train, labels_train, datasets_train_utility = (
    ...
    datasets_test, labels_test, _ = create_shadow_training_data_membership(
        df=df_test,
        meta_data=meta_data,
        target_record=target_record,
        generator=generator,
    ...
    if COMPUTE_UTILITY:
        ...
        train_utility = compute_utility(
            datasets=datasets_train_utility[:10],
    ...
    if NAIVE_ATTACK:
        ...
        X_test, y_test = apply_feature_extractor(
            datasets=datasets_test.copy(),
            target_record=target_record,
            labels=labels_test,
            ohe=ohe,
            ohe_columns=categorical_cols,
        ...
        if FEAT_SELECTION:
            ...
            trained_models, all_results = fit_validate_classifiers(
                X_train, y_train, X_test, y_test, models=MODELS, cv=CV
            )

            NAIVE_OUTPUT_PATH = (
                f"{OUTPUT_DIR}/output_{datetime.datetime.now()}_naive.csv"
            )
    ...
    if QUERY_ATTACK:
        ...
        print("Fitting classifiers on query-based features...")
        trained_models, all_results = fit_validate_classifiers(
            X_train, y_train, X_test, y_test, models=MODELS, cv=CV
        )
    ...
    if TARGET_ATTENTION_ATTACK:
        print("Running target attention attack...")
        TARGET_ATTENTION_MODEL_PARAMS = {
            "num_in_features": len(ohe_column_names) + len(continuous_cols) + 2,
            "embedding_hidden_size": 10,
            "embedding_size": 10,
            "attention_size": 10,
            "n_records": 1000,
            "prediction_hidden_size": 5,
            "dropout_rate": 0.15,
        }

        trained_models, all_results = fit_set_based_classifier(
            all_datasets_train=datasets_train.copy(),
            datasets_test=datasets_test.copy(),
            all_labels_train=labels_train,
            ...
            model_type="TargetAttention",
            top_X=100,
        )

        TARGETATTENTION_OUTPUT_PATH = (
            f"{OUTPUT_DIR}/output_{datetime.datetime.now()}_targetattention.csv"
        )
        save_output(
            TARGETATTENTION_OUTPUT_PATH, vars(args), all_results, utility_results
        )
        
    total_time = time.time() - start_total
    overhead_time = total_time - generation_time
    
    # store with json in f"{OUTPUT_DIR}/output_{datetime.datetime.now()}_times.csv"
    times_output_path = f"{OUTPUT_DIR}/times.json"
    times_output = {
        "generation_calls": (N_POS_TRAIN + N_POS_TEST) * 2,
        "generation_time": generation_time,
        "overhead_time": overhead_time,
        "total_time": total_time,
    }
    with open(times_output_path, "w") as f:
        json.dump(times_output, f)
