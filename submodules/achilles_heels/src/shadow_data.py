...
def create_shadow_training_data_membership(df: pd.DataFrame, meta_data: list,
    ...
    # check how many times the target record is in the synthetic datasets
    count_in_synth = pd.Series([dataset.eq(target_record.iloc[0]).all(axis=1).any() for dataset in datasets]).mean()
    print(f"Target record is in {count_in_synth:.2%} of ynthetic datasets.")
