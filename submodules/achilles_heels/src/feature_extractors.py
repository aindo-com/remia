...
def discrete_features_bugfix(synthetic_df: pd.DataFrame, target_record: pd.DataFrame) -> tuple:
    '''
    C QBS only works with discrete features, so we need to make sure that all features are discrete. We do this by
    multiplying all continuous features with 100 and rounding them to the nearest integer. This is a bugfix for the
    feature extractor using C QBS, which was not working properly with continuous features.
    '''
    synthetic_df_discrete = synthetic_df.copy()
    target_record_discrete = target_record.copy()
    for col in synthetic_df.columns:
        if synthetic_df[col].dtype == 'float64' and (synthetic_df[col]<2).all():
            synthetic_df_discrete[col] = (synthetic_df[col] * 100).round()
            target_record_discrete[col] = (target_record[col] * 100).round()

    return synthetic_df_discrete.astype(int), target_record_discrete.astype(int)

def feature_extractor_queries_CQBS(synthetic_df: pd.DataFrame, target_record: pd.DataFrame,
                                   queries: list):
    
    discretized_synthetic_df, discretized_target_record = discrete_features_bugfix(synthetic_df, target_record)
    
    # set up qbs of synthetic dataframe and define target values
    qbs_data = qbs.SimpleQBS(discretized_synthetic_df.itertuples(index=False, name=None))
    target_values = [tuple(discretized_target_record.values[0])]
...
def apply_feature_extractor(datasets: list, target_record: pd.DataFrame, labels: list, ohe: OneHotEncoder,
    ...
    for k, dataset in tqdm.tqdm(enumerate(datasets), total=len(datasets), desc='Extracting features'):
        if sum(do_ohe) != 0:
