...
from abc import ABC
from typing import Generic, Literal, TypeVar, cast, ClassVar
...
class BaseSingleColumnTechnique(BaseTechnique, ABC):
    ...
    def anonymize(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            self.anonymize_column(dataframe.iloc[:, 0]), columns=dataframe.columns
        )
    ...
    def _apply_to_col(self, col: pd.Series) -> pd.Series:
        ...
        if self.sampling_mode == "weighted":
            ...
            transformer = QuantileTransformer(
                ...
                n_quantiles=min(1_000, col.shape[0]),
            ...
            col_r = (
                np.sqrt(1 - self.alpha) * col_r + np.sqrt(self.alpha) * random_values
            )
            ...
            random_values: np.ndarray = self.generator.uniform(
                low=min_, high=max_, size=col.size
            )
