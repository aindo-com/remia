...
def compute_distances(record: np.array, values: np.array,
    ...
    # PATCH: if there are no categorical columns, skip the categorical distance entirely and return only the continuous distance, weighted by the number of continuous columns (since the final distance is a weighted average of the two distances, weighted by the number of respective columns)
    # first define distance based on categorical
    if n_cat_cols == 0:
        # skip categorical distance entirely
        if method == 'cosine':
    ...
    # PATCH: same for continuous
    if n_cont_cols == 0:
        # skip continuous distance entirely
        if method == 'cosine':
